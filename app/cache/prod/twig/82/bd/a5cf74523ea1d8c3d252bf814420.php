<?php

/* ::error.html.twig */
class __TwigTemplate_82bda5cf74523ea1d8c3d252bf814420 extends Twig_Template
{
    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->parent = false;

        $this->blocks = array(
        );
    }

    protected function doDisplay(array $context, array $blocks = array())
    {
        // line 1
        if ($this->getAttribute($this->getAttribute((isset($context["app"]) ? $context["app"] : null), "session"), "get", array(0 => "last_error"), "method")) {
            // line 2
            echo "  <div class=\"alert alert-error alert-block\">  
    <a class=\"close\" data-dismiss=\"alert\">×</a>  
    <strong>Warning!</strong>
    ";
            // line 5
            echo twig_escape_filter($this->env, $this->getAttribute($this->getAttribute($this->getAttribute((isset($context["app"]) ? $context["app"] : null), "session"), "get", array(0 => "last_error"), "method"), "error"), "html", null, true);
            echo "
  </div>
";
        }
    }

    public function getTemplateName()
    {
        return "::error.html.twig";
    }

    public function isTraitable()
    {
        return false;
    }

    public function getDebugInfo()
    {
        return array (  19 => 2,  52 => 26,  44 => 21,  28 => 10,  26 => 9,  153 => 65,  147 => 61,  141 => 57,  135 => 56,  132 => 55,  126 => 51,  120 => 50,  115 => 48,  109 => 47,  103 => 44,  97 => 43,  75 => 37,  70 => 35,  64 => 34,  57 => 30,  53 => 28,  51 => 27,  36 => 15,  24 => 5,  23 => 5,  25 => 7,  17 => 1,  241 => 59,  237 => 38,  151 => 36,  146 => 21,  143 => 20,  139 => 18,  113 => 16,  108 => 11,  105 => 10,  99 => 5,  94 => 63,  92 => 41,  88 => 60,  86 => 40,  83 => 58,  81 => 38,  76 => 54,  74 => 53,  71 => 52,  68 => 51,  65 => 37,  62 => 49,  59 => 31,  56 => 47,  54 => 46,  48 => 43,  42 => 39,  40 => 20,  37 => 19,  21 => 1,  79 => 16,  39 => 12,  35 => 15,  30 => 4,  27 => 5,);
    }
}
